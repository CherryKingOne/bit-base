...
class VllmEngine(BaseEngine):
    ...
    def load_model(self, model_path: Path, precision: str, **kwargs) -> None:
        self._port = kwargs.get("port", 8000)
        ...
        dtype = dtype_map.get(precision, "auto")
        if dtype != "auto":
            cmd.extend(["--dtype", dtype])

        # 量化
        if precision.startswith("int") or precision.startswith("gptq"):
            cmd.extend(["--quantization", precision])
    ...
    def _wait_ready(self, timeout: int = 120) -> None:
        ...
        while time.time() - start < timeout:
            try:
                ...
                pass
            time.sleep(1)
        raise TimeoutError(f"vLLM 启动超时 ({timeout}s)")
